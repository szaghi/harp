...
`.hrz` mask), `harp when` (rank the coming nights for one target), `harp sites` (manage
saved observing sites), `harp log` (record what you actually imaged),
`harp list` (show configured sites/optics).
